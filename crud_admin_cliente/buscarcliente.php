...
<head>
    ...
    <title>CRUD Admin - Cliente</title>
</head>
...
<body>
    ...
    if (isset($_POST['buscarcliente'])) {
        ...
        $_SESSION['buscarcliente'] = $buscar;
    } else
        $buscar = $_SESSION['buscarcliente'];
    ?>
            ...
            <div class="col-sm-6">
                <form method="POST" action="">
                    <div class="my-3">
                        ...
                        <input class="form-control" type="text" name="buscarcliente" id="" value="<?= $buscar ?>">
                        <input class="btn btn-primary my-1" type="submit" value="Buscar">
                        <a class="btn btn-danger my-1" href="crud_admin_cliente.php">Reset</a>
                    </div>
                </form>
            </div>
        </div>
        <div class="row">
            <div class="table-responsive">
                <table class="table">
                    ...
                    $select = "SELECT id, nombre, apellido, dni, telefono, mail, pass, sexo FROM clientes WHERE apellido LIKE '%$buscar%' OR nombre LIKE '%$buscar%' ORDER BY id DESC;";
                    ...
                    $busqueda = mysqli_query($conexion, "SELECT id, nombre, apellido, dni, telefono, mail, pass, sexo FROM clientes WHERE apellido LIKE '%$buscar%' OR nombre LIKE '%$buscar%' ORDER BY id DESC LIMIT $inicio, $num_registros;");
            ...
            <ul class="pagination pg-dark justify-content-center py-5 mb-0" style="float: none;">
                <li class="page-item">
                    ...
                    if ($_REQUEST['nume'] == "1") {
                        ...
                        // echo '<a class="page-link" aria-label="Anterior" href="buscarcliente.php?nume=1"><span aria-hidden="true">&laquo;</span><span class="sr_only">Anterior</span></a>';
                        echo '<li class="page-item"><a class="page-link" href="buscarcliente.php?nume=' . ($pagina - 1) . '">' . $anterior . '</a></li>';
                    }
                    ...
                    if ($pagina < $paginas && $paginas > 1)
                        echo '<li class="page-item"><a class="page-link" href="buscarcliente.php?nume=' . ($pagina + 1) . '">' . $siguiente . '</a></li>';
                    // echo '<li class="page-item"><a class="page-link" aria-label="Siguiente" href="buscarcliente.php?nume=' . ceil($ultima) . '"><span aria-hidden="true">&raquo;</span><span class="sr_only">Siguiente</span></a></li>';
                    ?>
                </li>
            </ul>
        </div>
